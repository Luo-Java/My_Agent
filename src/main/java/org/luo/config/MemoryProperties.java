...
/**
 * 会话记忆窗口配置（{@code agent.memory.*}）。
 * <p>
 * 原先这三项是 {@link DbChatMemory} 里的 {@code static final} 常量，改一次要重新编译；
 * 且其中「窗口无下限保护」是一个真实的静默缺陷——单条消息自己就超过预算时，
 * 窗口起点被推到末尾，<b>整个历史窗口塌缩为空</b>，模型看谁都是第一次对话
 * （见 {@link DbChatMemory#computeWindowStart}）。故抽成本配置类，把「保留条数下限」
 * 与「单条截断」做成可调 + 可关闭。
 * <p>
 * 三个参数共同决定 {@code DbChatMemory.get()} 返回哪一段历史，而
 * {@code MemoryMergeService} 用<b>同一个函数、同一份配置</b>计算合并边界——
 * 两处必须一致，否则「被摘要掉的区间」与「真正不进的上下文区间」会错位，
 * 表现为记忆重复或静默丢失。故本类被这两处共同注入，是唯一的参数来源。
 *
 * @param recentTokens   维持上下文的最大原文 token 预算（近似值，按字符数估算）；超出部分由滚动摘要覆盖
 * @param minKeepMessages 窗口<b>至少</b>保留的最近消息条数（不受 token 预算约束）——防塌缩下限；0=关闭
 * @param maxMessageChars 单条消息进入上下文前的字符上限，超出截断并在末尾标注；0=不截断
 */
...
public record MemoryProperties(Integer recentTokens, Integer minKeepMessages, Integer maxMessageChars) {
     ...
     * 默认上下文预算（字符近似）。
     * <p>
     * 数据类回复（分布表 / 成绩表）可达 1~3k 字符，1000 会把上一轮整体切掉、
     * 导致模型看不到已有数据而重复查库；4000 可在成本可控前提下保住 1~2 轮数据型对话。
     */
    public static final int DEFAULT_RECENT_TOKENS = 4000;

    /**
     * 默认最少保留条数 = 1 组「用户提问 + 助手回答」。
     * <p>
     * 保留 2 条而不是 1 条：只有 1 条时窗口里可能只剩半轮（最坏是只剩一条巨大的助手回答），
     * 模型看不到对应的提问，反而更容易答偏。
     */
