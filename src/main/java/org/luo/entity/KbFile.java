 ...
 * <p>
 * 文件是知识的上传 / 管理单元：上传文件 → 解析文本 → 分块向量化写入 {@code kb_chunk}（每块 source = 文件名），
 * 同时在此登记一条记录（大小 / 分块数等）。删除文件 = 删该记录并级联删除其名下全部知识块。同库内文件名唯一
...
public class KbFile {
    ...
    /** 文件名（含扩展名；同库内唯一，与名下知识块的 source 一致，用于级联删除）。 */
    ...
    /** 相邻块重叠字符数（0 = 不重叠；默认 60；重新分片时沿用该值，除非显式改传）。 */
    ...
    /** 入库时的解析原文：用于不重传文件直接切换分片策略（重新分片）；仅写不读（列表接口不回传）。 */
